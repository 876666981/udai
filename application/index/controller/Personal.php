...
class Personal extends Controller
{
    ...
    public function udai_collection(){
            ...
            $id=Session::get('id');
        $controller=db('Collection')->where('pid',$id)->select();
    ...
    public function udai_order(){
        ...
        $order=db('order')->where('uid',$list['id'])->where('strike',0)->order('id desc')->paginate(3);
        $page=$order->render();
        $arr=array();
    ...
    public function dopass(){
        ...
        if ($pass['password']==$pass['repassword']) {
            ...
            $m=db('user')->where('phone',$phone)->update(['password'=>$password]);
        }
        return $m;
    }
    ...
    public function do_tel(){
        if (Request::instance()->isPost()) {
            ...
            if(($userInfo['phone'] == $phone) && ($smscode==Session::get('param'))) {
                return "OK";
            }
        }
    }
